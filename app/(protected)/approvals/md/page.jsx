...
export default function MDApprovalsPage() {
  ...
  async function act(id, action) {
    ...
    try {
      if (action === "approve")
        await apiFetch(`/api/evals/${id}/md-approve`, { method: "POST", body: { comment: "" } });
      else
          ...
          {list.map(ev => (
            <tr key={ev.id} className="border-b">
              ...
              <td className="flex gap-2 py-2">
                <button className="px-3 py-1 rounded bg-purple-700 text-white disabled:opacity-50" disabled={busy} onClick={()=>act(ev.id,"approve")}>MD Approve</button>
