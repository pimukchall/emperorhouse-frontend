...
export default function AdminDepartmentsPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await apiFetch(`/api/departments?q=${encodeURIComponent(q)}&page=1&limit=200&sortBy=id&sort=asc`);
      setItems(res?.data || []);
      setError("");
    } catch (e) {
  ...
  return (
    <main className="pt-20 px-4 sm:px-6 lg:px-8 max-w-screen-xl mx-auto space-y-4">
      <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <h1 className="text-xl sm:text-2xl font-semibold text-neutral-800 dark:text-neutral-100">Departments</h1>
        <div className="flex flex-wrap gap-3 items-center">
          <input className="w-52 rounded-md border px-3 py-2 text-sm dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-200" placeholder="ค้นหา department" value={q} onChange={(e)=>setQ(e.target.value)} />
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-neutral-100 dark:bg-neutral-800">
              <tr><Th>ID</Th><Th>Code</Th><Th>Name (TH)</Th><Th>Name (EN)</Th><Th right>Actions</Th></tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                items.map((d, idx) => (
                  <tr key={d.id} className={`border-t dark:border-neutral-700 ${idx%2===0 ? "bg-neutral-50 dark:bg-neutral-900/50":""} hover:bg-neutral-100 dark:hover:bg-neutral-800`}>
                    <Td>{d.id}</Td>
                    ...
                    <Td right>
                      <div className="flex gap-1 justify-end">
                        ...
                        <GhostButton className="text-rose-600" onClick={()=>remove(d.id)}>Delete</GhostButton>
                      </div>
                    </Td>
                  </tr>
                ))
