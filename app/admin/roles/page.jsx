...
export default function AdminRolesPage() {
  ...
  const [form, setForm] = useState({ id: null, name: "", labelTh: "", labelEn: "" });
  ...
  async function load() {
    setBusy(true);
    try {
      const res = await apiFetch(`/api/roles?q=${encodeURIComponent(q)}&page=${page}&limit=${PAGE_SIZE}`);
      setItems(res?.data || []);
  ...
  return (
    <main className="mx-auto max-w-5xl p-6 py-30 space-y-8">
      ...
      <section className="rounded-xl border p-4">
        <h2 className="font-medium mb-3">{isEditing ? "แก้ไข Role" : "เพิ่ม Role ใหม่"}</h2>
        <form className="grid gap-3 sm:grid-cols-3" onSubmit={onSubmit}>
          <label className="block">
            <div className="text-xs">name (unique)</div>
            <input
              value={form.name}
          ...
          <label className="block">
            <div className="text-xs">labelTh</div>
            <input
              value={form.labelTh}
          ...
          <label className="block">
            <div className="text-xs">labelEn</div>
            <input
              value={form.labelEn}
          ...
          <table className="w-full text-sm">
            <thead className="bg-neutral-50">
              <tr>
                ...
                <th className="px-3 py-2 text-left">labelTh</th>
                <th className="px-3 py-2 text-left">labelEn</th>
            ...
            <tbody>
              {items.map((r) => (
                <tr key={r.id} className="border-t">
                  ...
                  <td className="px-3 py-2">{r.labelEn || "-"}</td>
                  <td className="px-3 py-2 text-right">
                    <button
                      className="px-2 py-1 text-blue-600"
                      onClick={() => setForm({ id: r.id, name: r.name, labelTh: r.labelTh || "", labelEn: r.labelEn || "" })}
                    >
                      Edit
                    </button>
